...
function solution(N) {
  ...
  while (i < length && j < length) {
    ...
    if (i === -1) break;
    /*
        binary gap is sequence of zeroes surrounded by 1 at both ends
        If we don't get the next 1, hence there is no binary gap
      */
    j = getNext1(binaryArray, i + 1);
    if (j === -1) break;
    // j-i-i because we don't want to include start and end elements in our range
